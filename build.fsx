...
let reloadScript () =
  try
    ...
    let appFsx = __SOURCE_DIRECTORY__ @@ "src" @@ "server" @@ "server.fsx"
...
// --------------------------------------------------------------------------------------
// Suave server that redirects all requests to currently loaded version
// --------------------------------------------------------------------------------------
...
let serverConfig =
  { defaultConfig with
      homeFolder = Some (__SOURCE_DIRECTORY__ @@ "src" @@ "server" @@ "public")
...
let startNpmWatcher () = 
  async {
    return Npm (fun p -> { p with Command = Run "develop" } )
  } |> Async.Start 
...
Target "develop" (fun _ ->
  ...
  startNpmWatcher ()
  use watcher = !! (__SOURCE_DIRECTORY__ @@ "src" @@ "server" @@ "*.*") |> WatchChanges (fun _ -> reloadAppServer())
